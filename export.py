...
import argparse, asyncio, base64, json, os, re, shutil, subprocess, sys, tempfile
...
def _pptx_to_thumbnails(pptx_path: str, tmpdir: str) -> list[str]:
    """Convert a .pptx to per-slide PNG thumbnails.

    Strategy: LibreOffice converts PPTX→PDF (one page per slide), then pdftoppm
    renders each PDF page as a PNG. Falls back to direct PNG export (single image)
    ...
    if pdftoppm:
        # Step 1: PPTX → PDF (LibreOffice preserves all slides as PDF pages)
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(f"_pptx_to_thumbnails: soffice PDF failed: {result.stderr.decode()}")
                return []
        except Exception as e:
            logger.error(f"_pptx_to_thumbnails: soffice PDF error: {e}")
            return []

        pdf_files = [os.path.join(pdf_dir, f) for f in os.listdir(pdf_dir) if f.lower().endswith(".pdf")]
        if not pdf_files:
            logger.error("_pptx_to_thumbnails: no PDF generated by soffice")
            return []
        ...
        try:
            result = subprocess.run(
                [pdftoppm, "-png", "-r", "150", pdf_files[0], png_prefix],
                capture_output=True, timeout=120,
            )
            if result.returncode != 0:
                logger.error(f"_pptx_to_thumbnails: pdftoppm failed: {result.stderr.decode()}")
                return []
        except Exception as e:
            logger.error(f"_pptx_to_thumbnails: pdftoppm error: {e}")
            return []
    else:
        # Fallback: direct PNG export — LibreOffice only renders the first slide
        logger.warning("_pptx_to_thumbnails: pdftoppm not found, visual QA will only see slide 1")
        try:
            result = subprocess.run(
            ...
            if result.returncode != 0:
                logger.error(f"_pptx_to_thumbnails: soffice PNG failed: {result.stderr.decode()}")
                return []
        except Exception as e:
            logger.error(f"_pptx_to_thumbnails: soffice PNG error: {e}")
            return []
...
def _assemble_pptx(deck_spec: dict, output: str) -> None:
    ...
    # Force normal (slide editor) view instead of slide sorter
    from pptx.oxml.ns import qn
    prs_elem = prs.element
    view_pr = prs_elem.find(qn('p:viewPr'))
    if view_pr is None:
        view_pr = prs_elem.makeelement(qn('p:viewPr'))
        prs_elem.append(view_pr)
    view_pr.set('lastView', 'sldView')
